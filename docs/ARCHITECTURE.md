...
Current verification does not include a real PresentMon ETW capture or a full Three.js-to-Unity project supplied by the user. The included smoke scenario is Web-vs-Web and verifies the Web collection/report path only. Unity `6000.3.22f1` batchmode compilation has been exercised; Unity `2021.3` is declared by the UPM package but has only static compatibility review in this repository.
